...
class TranscriptRAG:
    ...
    def _rerank_results(
        ...
        for idx, item in enumerate(candidates):
            ...
            # L2 distance from FAISS. Smaller distance means higher semantic similarity.
            distance = float(item.get("distance", 1e6))
            semantic = 1.0 / (1.0 + max(distance, 0.0))

            # Rank prior keeps earlier semantic neighbors slightly preferred.
            rank_prior = 1.0 - (idx / max(total, 1))

            score = 0.70 * semantic + 0.25 * lexical + 0.05 * rank_prior
